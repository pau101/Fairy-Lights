...
public class RecipeLight implements IRecipe {
	...
	public RecipeLight(LightVariant variant) {
		recipeOutput = variant.getCraftingResult();
		Object[] recipeObjects = variant.getCraftingRecipe();
		...
		if (recipeObjects[i] instanceof String[]) {
			...
			for (int l = 0; l < craftingLines.length; l++) {
				String row = craftingLines[l];
				recipeHeight++;
		...
		for (i = 0; i < recipeWidth * recipeHeight; ++i) {
			...
			if (charMap.containsKey(Character.valueOf(recipeSymbol))) {
		...
		for (int x = 0; x < 3; x++) {
			for (int y = 0; y < 3; y++) {
				...
				if (stackInCraftingTable != null || target != null) {
					if (stackInCraftingTable == null && target != null || stackInCraftingTable != null && target == null) {
						return false;
					}
