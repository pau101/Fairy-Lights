...
public final class GuiEditLetteredConnection<C extends Connection & Lettered> extends GuiScreen {
	...
	public void initGui() {
		...
		buttonList.clear();
		doneBtn = func_189646_b(new GuiButton(DONE_ID, width / 2 - pad - buttonWidth, height / 4 + 120 + 12, buttonWidth, 20, I18n.format("gui.done")));
		cancelBtn = func_189646_b(new GuiButton(CANCEL_ID, width / 2 + pad, height / 4 + 120 + 12, buttonWidth, 20, I18n.format("gui.cancel")));
		int textFieldX = width / 2 - 150, textFieldY = height / 2 - 10;
		int buttonX = textFieldX, buttonY = textFieldY - 25, bInc = 24;
		colorBtn = func_189646_b(new GuiButtonColor(COLOR_ID, buttonX, buttonY));
		paletteBtn = func_189646_b(new GuiButtonPalette(PALETTE_ID, buttonX - 4, buttonY - 30, ClientProxy.recoloredFont, colorBtn));
		boldBtn = func_189646_b(new GuiButtonToggle(BOLD_ID, buttonX += bInc, buttonY, 40, 0));
		italicBtn = func_189646_b(new GuiButtonToggle(ITALIC_ID, buttonX += bInc, buttonY, 60, 0));
		underlineBtn = func_189646_b(new GuiButtonToggle(UNDERLINE_ID, buttonX += bInc, buttonY, 80, 0));
		strikethroughBtn = func_189646_b(new GuiButtonToggle(STRIKETHROUGH_ID, buttonX += bInc, buttonY, 100, 0));
