...
public class TileEntityFairyLightsFastener extends TileEntity {
	...
	public TileEntityFairyLightsFastener() {
	...
	public boolean hasConnectionWith(TileEntityFairyLightsFastener fastener) {
		for (Connection connection : connections.values()) {
	...
	public void connectWith(EntityPlayer entity, NBTTagCompound tagCompound) {
		connections.put(UUID.randomUUID(), new ConnectionPlayer(this, worldObj, entity, tagCompound));
	}

	public void connectWith(TileEntityFairyLightsFastener fairyLightsFastener, NBTTagCompound tagCompound) {
		connections.put(UUID.randomUUID(), new ConnectionFastener(this, worldObj, fairyLightsFastener.xCoord, fairyLightsFastener.yCoord,
			fairyLightsFastener.zCoord, true, tagCompound));
		fairyLightsFastener.connections.put(UUID.randomUUID(), new ConnectionFastener(this, worldObj, xCoord, yCoord, zCoord, false, tagCompound));
	}

	public Point3f getConnectionPoint() {
		if (connectionPoint == null) {
			connectionPoint = ((BlockFairyLightsFastener) getBlockType()).getOffsetForData(getBlockMetadata(), 0.125f).add(xCoord, yCoord, zCoord);
		}
	...
	public AxisAlignedBB getBoundingBox() {
		return AxisAlignedBB.getBoundingBox(min.x / 16, min.y / 16, min.z / 16, max.x / 16, max.y / 16, max.z / 16).expand(10, 10, 10)
	...
	public void removeConnection(Entity entity) {
		for (Connection connection : connections.values()) {
			if (connection instanceof ConnectionPlayer) {
				if (entity.getUniqueID().equals(((ConnectionPlayer) connection).getPlayerUUID())
					|| entity instanceof EntityPlayer
					&& UUID.nameUUIDFromBytes(("OfflinePlayer:" + ((EntityPlayer) entity).getGameProfile().getName()).getBytes(Charsets.UTF_8)).equals(
						((ConnectionPlayer) connection).getPlayerUUID())) {
	...
	public void updateEntity() {
		if (!(worldObj.getBlock(xCoord, yCoord, zCoord) instanceof BlockFairyLightsFastener)) {
			return;
		}
		if (!worldObj.isRemote) {
			if (connections.size() == 0) {
				if (getBlockType() == FairyLights.fairyLightsFastener) {
					worldObj.setBlockToAir(xCoord, yCoord, zCoord);
				} else if (getBlockType() == FairyLights.fairyLightsFence) {
		...
		Point3f fromOffset = ((BlockFairyLightsFastener) getBlockType()).getOffsetForData(worldObj.getBlockMetadata(xCoord, yCoord, zCoord), 0.125f).add(xCoord, yCoord, zCoord);
