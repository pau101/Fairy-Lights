			...
			if (FairyLights.christmas.isOcurringNow() && Configurator.jingleEnabled && random.nextFloat() < jingleProbability) {
				...
				if (lightPoints != null) {
					...
					if (jingle != null) {
						...
						FairyLights.networkManager.sendPacketToClientsWatchingChunk(fastener.getPos().getX(), fastener.getPos().getZ(), event.world, new S00FLPacketJingle(fastener, uuid, lightOffset, jingle));
					}
				}
			}
		}
