...
/** Catenary
 * 
 * Consider changing any references of (float) Math.sqrt to a different, faster method.
 */
public final class Catenary {
    ...
    public Catenary lerp(final Catenary other, final float delta) {
        ...
        final float angle = FLMath.lerpAngle(this.yaw, other.yaw, delta);
    ...
    public static Catenary from(final Vector3d direction, final float verticalYaw, final CubicBezier bezier, final float slack) {
        final float dist = (float) VectorUtils.length(direction);
        final float length;
    ...
    public static Catenary from(final Vector3d dir, final float verticalYaw, final float ropeLength) {
        final float endX = (float) Math.sqrt(dir.x * dir.x + dir.z * dir.z);
        ...
        final float vx = Mth.cos(angle);
        final float vz = Mth.sin(angle);
        if (VectorUtils.length(dir) > 2.0F * Connection.MAX_LENGTH) {
            return new Catenary(2, angle, vx, vz, new float[]{0.0F, endX}, new float[]{0.0F, endY}, (float) Math.sqrt(endX * endX + endY * endY));
        }
