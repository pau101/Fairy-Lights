...
public final class CatenaryUtils {
    ...
    private static void catenary(float x1, float y1, float x2, float y2, final float length, final int pointCount, final float[] x, final float[] y, float sag) {
        ...
        if (MathHelper.func_76135_e(d) < MIN_HORIZ || length <= MathHelper.func_76129_c(d * d + h * h)) {
        ...
        for (int iter = 0; iter < MAX_ITER; iter++) {
            ...
            if (MathHelper.func_76135_e(val) < MIN_VAL || MathHelper.func_76135_e(grad) < MIN_GRAD) {
                break;
            }
            final float search = -g(sag, d, length, h) / dg(sag, d);
            float alpha = 1;
            float sagNew = sag + alpha * search;
            final float valAbs = MathHelper.func_76135_e(val);
            while (sagNew < 0 || MathHelper.func_76135_e(g(sagNew, d, length, h)) > valAbs) {
    ...
    private static float g(final float s, final float d, final float length, final float h) {
        return 2 * (float) Math.sinh(s * d / 2) / s - MathHelper.func_76129_c(length * length - h * h);
    }
