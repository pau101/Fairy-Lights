...
public final class OreDictUtils {
    ...
    public static int getDyeMetadata(final ItemStack stack) {
        if (!stack.isEmpty()) {
            if (stack.getItem() instanceof DyeItem) {
    ...
    private static ImmutableMultimap<Integer, ItemStack> getDyeItemStacks() {
        final ImmutableMultimap.Builder<Integer, ItemStack> bob = ImmutableMultimap.builder();
        for (final Dye dye : Dye.values()) {
