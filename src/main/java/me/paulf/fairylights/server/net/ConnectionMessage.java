...
public abstract class ConnectionMessage<C extends Connection> {
	...
	public ConnectionMessage(C connection) {
	...
	public static void serialize(ConnectionMessage message, PacketBuffer buf) {
		buf.writeBlockPos(message.pos);
		buf.writeCompoundTag(FastenerType.serialize(message.accessor));
		buf.writeUniqueId(message.uuid);
	}

	public static void deserialize(ConnectionMessage message, PacketBuffer buf) {
	...
	@Nullable
	public static <C extends Connection> C getConnection(ConnectionMessage<C> message, Predicate<? super Connection> typePredicate, World world) {
