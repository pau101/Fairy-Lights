...
public abstract class ConnectionItem extends Item {
	public ConnectionItem(Properties properties) {
		super(properties);
	}
	...
	public ActionResultType onItemUse(final ItemUseContext context) {
		...
		BlockState fastenerState = fastener.getDefaultState().with(FastenerBlock.FACING, side);
		...
		if (currentBlockState.getBlock() == fastener) {
			...
			if (entity == null || entity instanceof FenceFastenerEntity) {
				if (!world.isRemote) {
					connectFence(stack, user, world, clickPos, (FenceFastenerEntity) entity);
				}
				return ActionResultType.SUCCESS;
			}
		}
