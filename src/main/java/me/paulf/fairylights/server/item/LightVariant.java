...
public enum LightVariant {
	...
	private Supplier<RegistryObject<? extends Item>> item;
	...
	private LightVariant(String name, Supplier<RegistryObject<? extends Item>> item, boolean parallelsCord, float width, float height, UnaryOperator<GenericRecipeBuilder> recipe, Placement orientable) {
		this(name, item, parallelsCord, 16, width, height, recipe, orientable);
	}

	private LightVariant(String name, Supplier<RegistryObject<? extends Item>> item, boolean parallelsCord, float spacing, float width, float height, UnaryOperator<GenericRecipeBuilder> recipe, Placement orientable) {
		this(name, item, parallelsCord, spacing, width, height, recipe, 0.05F, 40, false, orientable);
	}

	private LightVariant(String name, Supplier<RegistryObject<? extends Item>> item, boolean parallelsCord, float spacing, float width, float height, UnaryOperator<GenericRecipeBuilder> recipe, float twinkleChance, int tickCycle, Placement orientable) {
		this(name, item, parallelsCord, spacing, width, height, recipe, twinkleChance, tickCycle, true, orientable);
	}

	private LightVariant(String name, Supplier<RegistryObject<? extends Item>> item, boolean parallelsCord, float spacing, float width, float height, UnaryOperator<GenericRecipeBuilder> recipe, float twinkleChance, int tickCycle, boolean alwaysTwinkle, Placement orientable) {
		this.name = name;
		this.item = item;
	...
	public Item getItem() {
		return item.get().orElseThrow(IllegalStateException::new);
	}
