...
public class GhostLightModel extends ColorLightModel {
    public GhostLightModel() {
        ...
        littleFace.field_78795_f = FLMath.PI;
        littleFace.field_78796_g = FLMath.PI;
        ...
        bodyTop.setAngles(FLMath.PI, 0.0F, 0.0F);
        final BulbBuilder body = bulb.createChild(46, 40);
        body.setPosition(0.0F, 1.0F, 0.0F);
        body.addBox(-2.0F, 0.0F, -2.0F, 4.0F, 4.0F, 4.0F);
        body.setAngles(FLMath.PI, 0.0F, 0.0F);
        final Vector3f vec = new Vector3f(-1.0F, 0.0F, 1.0F);
        vec.func_229194_d_();
        final Quaternion droop = vec.func_229193_c_(-FLMath.PI / 3.0F);
        final int finCount = 8;
        for (int i = 0; i < finCount; i++) {
            final BulbBuilder fin = bulb.createChild(40, 21);
            final Quaternion q = Vector3f.field_229181_d_.func_229193_c_(i * FLMath.TAU / finCount);
            q.func_195890_a(droop);
            final float[] magicAngles = toEuler(q);
            final float theta = i * FLMath.TAU / finCount;
            fin.setPosition(MathHelper.func_76134_b(-theta + FLMath.PI / 4) * 1.1F, -2.75F, MathHelper.func_76126_a(-theta + FLMath.PI / 4.0F) * 1.1F);
